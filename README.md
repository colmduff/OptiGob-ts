...
- **Data**: `resource_manager/database_manager.py` queries a bundled SQLite database (`src/optigob_ts/database/optigob_ts_default_0.1.0.db`) for baseline values and scenario scalers. Pass a custom `db_file_path` to `Optigob(...)` to use a different database with the same schema.
